...
module compressor_n_2 #(
    ...
    generate
        ...
        for (stage = 0; stage < STAGE_NUM; stage++) begin : gen_stages
            ...
            localparam int WIDTH_OUT      = WIDTH_IN + 2;
            ...
            if (!IS_FIRST_STAGE && (REM > 0)) begin
                assign tmp[stage][BASE_OUT+2][WIDTH_OUT-1:0] = {{2{tmp[stage-1][BASE_IN+4][WIDTH_IN-1]}}, tmp[stage-1][BASE_IN+4][WIDTH_IN-1:0]};
                assign tmp[stage][BASE_OUT+3][WIDTH_OUT-1:0] = {{2{tmp[stage-1][BASE_IN+5][WIDTH_IN-1]}}, tmp[stage-1][BASE_IN+5][WIDTH_IN-1:0]};
            end
        end
